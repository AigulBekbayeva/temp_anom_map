...
Карта рендерится в canvas в проекции Web Mercator с билинейной интерполяцией и
кладётся на подложку как image overlay — поэтому 36 лет переключаются мгновенно
и без тайлового сервера.
...
- Июль текущего года попадает в архив с задержкой около 5 дней; если в поле пропуски,
  используйте `MODEL = "era5_seamless"`.
- `era5_land` даёт 0.1° и лучше прорисовывает горы, но не покрывает Каспий, Арал и Балхаш —
  на карте они станут пустыми.
- Одна аномалия — не тренд. Отдельный тёплый июль сам по себе ничего не доказывает;
  смысл появляется в ряду по годам (`figures/anomaly_07_series.png`) и в скользящем среднем.
